...
const DATABASE_PATH = 'data/database/csvdb'
...
function stringifyRows(rows: any[]) {
  ...
  const content = rows.map(row => headers.map(key => escapeCsvCell(row[key])).join(','))
  return [headers.map(escapeCsvCell).join(','), ...content].join('\n')
}
  ...
  for (const line of lines.slice(1)) {
    ...
    headers.forEach((header, index) => {
      ...
      if (value === 'true') {
        ...
        row[header] = false
      } else if (value && !Number.isNaN(Number(value)) && !value.includes('e') && !value.includes('x')) {
...
class CsvDBDriver extends Driver {
  ...
  async stats() {
    ...
    for (const table in sizes) {
      if (table in stats.tables) continue
      stats.tables[table] = {
        count: 0,
        size: sizes[table],
      }
